# claude-obsidian: Agent Instructions (archived legacy system)

This describes the transaction-based system that used to live at this
repository's root, before it was moved under `archive/`. The paths below
changed accordingly, but the instructions still apply verbatim to code and
docs under `archive/`. It is not wired up as active plugin skills — a
host's normal skill discovery only looks at `skills/`, not
`archive/skills/`; invoking one of these requires pointing the host at
`archive/skills/<name>/SKILL.md` explicitly.
...
Do not add mirrored files under `commands/`; the original design invoked
plugin skills by namespaced names such as `/claude-obsidian:wiki`.
